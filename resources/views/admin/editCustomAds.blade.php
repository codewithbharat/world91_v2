...
@section('content')
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            <div class="card-header " style="padding-block: 20px;">
                                <h3 class="card-title mb-0 fs-5">Edit Advertisement</h3>
                            </div>
                            <form class="form-group" method="post" action="{{ asset('ads/edit') }}/{{ $ad->id }}"
                                ...
                                <div class="card-body">
                                    <div class="form-group row">
                                        <div class="input-field">
                                            <input class="at-title" id="location" placeholder="" autocomplete="off"
                                                type="text" name="location" value="{{ $ad->location }}"
                                    ...
                                    <div class="form-group row">
                                        <div class="input-field col-md-6">
                                            <input autocomplete="off" type="text" name="GoogleClient" placeholder=""
                                                value="{{ $ad->google_client }}" id="GoogleClient"
                                        ...
                                        <div class="input-field col-md-6">
                                            <input autocomplete="off" type="text" name="GoogleSlot" placeholder=""
                                                value="{{ $ad->google_slot }}" id="GoogleSlot"
                                    ...
                                    <div class="form-group row">
                                        <div class="col-md-6 col-sm-12">
                                            ...
                                            <select class="js-example-basic-single form-select" id="ads_type"
                                                ...
                                                <option value="1"
                                                    {{ old('ads_type', $ad->ads_type) == '1' ? 'selected' : '' }}>
                                                    Google
                                                </option>
                                                <option value="0"
                                                    {{ old('ads_type', $ad->ads_type) == '0' ? 'selected' : '' }}>
                                                    Custom
                                                </option>
                                            </select>
                                            @error('ads_type')
                                        ...
                                        <div class="col-md-6 col-sm-12">
                                            ...
                                            <select class="js-example-basic-single form-select" id="page_type"
                                                ...
                                                @foreach (\App\Models\Ads::PAGE_TYPES as $type)
                                                    <option value="{{ $type }}"
                                                        {{ old('page_type', $ad->page_type ?? '') === $type ? 'selected' : '' }}>
                                                        {{ ucfirst($type) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <?php
                                    $imagePath = rtrim($ad->file_path, '/'); // remove trailing slash if any
                                    $imageName = ltrim($ad->custom_image, '/'); // remove leading slash if any
                                    ...
                                    <div class="row align-items-center justify-content-center mb-3" style="gap: 20px;">
                                        ...
                                        <div class="col-auto p-0">
                                            <img id="image-preview"
                                                src="{{ $imageFilePath ? asset($imageFilePath) : '#' }}" alt="Preview"
                                                style="{{ $imageFilePath ? '' : 'display: none;' }} max-width: 100%; max-height: 100%; border-radius: 6px; border: 1px solid #ddd; background: #f8f8f8;" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="uploads-container row">
                                        <div class="uploads col-md-5" id="image-upload-section"
                                            ...
                                            <div class="uploads-box">
                                                ...
                                                @if (isset($ad->custom_image))
                                                    <p>Saved Image: <strong>{{ $ad->custom_image }}</strong></p>
                                                @endif
                                                <label for="image_file" class="drop-container" style="margin-top: 40px;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (imageInput && preview) {
                // Show the existing image preview on load if it has a valid src
    ...
    <script>
        window.existingImage = {{ !empty($ad->custom_image) && !empty($ad->file_path) ? 'true' : 'false' }};
    </script>
    ...
    <script>
        function validateEditForm() {
            ...
            // Use server-side injected value (must be boolean true/false)
            const existingImage = window.existingImage === true;

            // Clear previous error messages
            const errorFields = ['location', 'ads_type', 'GoogleClient', 'GoogleSlot', 'link', 'image_file'];
