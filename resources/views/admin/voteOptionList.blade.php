                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center px-5 py-">
                                <h3 class="card-title mb-0 fs-5">Vote Option List</h3>
                                <div class="card-tool ">
                                    <div class="input-group input-group-sm float-right  ">
                                        <a href="{{ asset('vote/vote-option') }}/add" class="--btn"
                            ...
                            <div class="row pt-4 px-5">
                                ...
                                <div class="card-body table-responsive px-1 py-2">
                                    <table class="table text-nowrap webs-table">
                                        ...
                                        <tbody>
                                            @if (count($data['latest_option']) > 0)
                                                @foreach ($data['latest_option'] as $vote_option)
                                                    <tr>
                                                        <td>{{ $vote_option->id }}</td>
                                                        <td>{{ $vote_option->name }}</td>
                                                        <td>{{ $vote_option->vote_count }}</td>
                                                        <td class="text-nowrap">
                                                            <a class="act_btn"
                                                                href="{{ asset('vote/vote-option/edit') }}/{{ $vote_option->id }}?t={{time()}}"><i class="fa-solid fa-pen-to-square"></i></a>
                                                            {{-- <a class="act_btn"
                                                                href="{{ asset('vote-option/delete/') }}/{{ $vote_option->id }}" onclick="openDeleteModal(this,'Are you sure you want to delete this item permanently?'); return false;"><i class="fa-solid fa-trash"></i></a> --}}
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            @else
                                ...
                                <div class="row pagination-block">
                                    <div class="col-sm-12 col-md-5">
                                        <div class="dataTables_info" id="dataTableExample_info" role="status"
                                            aria-live="polite">
                                            @if ($data['latest_option']->count() > 0)
                                                Showing {{ $data['latest_option']->firstItem() }} to
                                                {{ $data['latest_option']->lastItem() }} of
                                                {{ $data['latest_option']->total() }} entries
                                            @else
                                                Showing 0 entries
                                            @endif
                                        </div>
