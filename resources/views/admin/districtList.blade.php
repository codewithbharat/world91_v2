                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center px-5 py-2.5">
                                <h3 class="card-title mb-0 fs-5">District LIST</h3>

                                <div class="card-tool ">
                                    <div class="input-group input-group-sm float-right  ">

                                        <a href="{{ asset('/district') }}/add" class="--btn" style="background: #28364f">
                                            ADD District
                                        </a>
                                        ...
                                        @if (count($districts) > 0)
                                            @foreach ($districts as $district)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a class="act_btn"
                                                            href="{{ asset('district/edit') }}/{{ $district->id }}"><i
                                                                class="fa-solid fa-pen-to-square"></i></a>
                                                        {{-- <a class="act_btn" href="{{ asset('district/delete') }}/{{$district->id}}"><i class="fa-solid fa-trash"></i></a> --}}
                                                    </td>
                                                    <td class="mx-auto">
                                                        <div class="form-check form-switch ms-4">
                                                            <input class="form-check-input" type="checkbox"
                                                                id="toggleActiveStatus{{ $district->id }}"
                                                                name="active_status_{{ $district->id }}"
                                                                {{ $district->status == 1 ? 'checked' : '' }}
                                                                onchange="updateDistrictActiveStatus({{ $district->id }}, this.checked)">
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
...
@endsection
<script>
    function updateDistrictActiveStatus(districtId, isChecked) {
        const status = isChecked ? 1 : 0;

        // Show loader overlay
        document.getElementById('loader-overlay').style.display = 'flex';

        fetch('/district/update-status', {
                method: 'POST',
                headers: {
