                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center px-5 py-">
                                <h3 class="card-title mb-0 fs-5">All HOME CATEGORY LIST</h3>
                                <div class="card-tool ">
                                    <div class="input-group input-group-sm float-right  ">
                                        <a href="{{ asset('home-category') }}/add" class="--btn"
                                            style="background: #28364f">
                                            ADD CATEGORY IN HOME
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div class="row pt-4 px-5">
                                ...
                                <div class="card-body table-responsive px-1 py-2 mt-1">
                                    ...
                                    <table class="table home-cat-list text-nowrap">
                                        <thead>
                                            <tr>
                                                <th>ID</th>
                                                <th>Title</th>
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($categories as $category)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span
                                                            class="status {{ $category->status == 1 ? 'status-active' : 'status-pending' }}">
                                                            {{ $category->status == 1 ? 'Active' : 'Inactive' }}
                                                        </span>
                                                    </td>
                                                    <td>{{ optional($category->category)->name }} ({{ $category->catid }})
                                                    </td>
                                                    <td>{{ $category->section_order }}</td>
                                                    <td>{{ $category->sidebar_sec_order }}</td>

                                                    <td>
                                                        <a class="act_btn"
                                                            href="{{ url('home-category/edit/' . $category->id) }}"><i
                                                                class="fa-solid fa-pen-to-square"></i>
                                                        </a>
                                                        {{-- <a class="act_btn"
                                                            href="{{ url('home-category/delete/' . $category->id) }}"
                                                    ...
                                                    <td class="mx-auto">
                                                        <div class="form-check form-switch ms-4">
                                                            <input class="form-check-input" type="checkbox"
                                                                ...
                                                                onchange="updateActiveStatus({{ $category->id }}, this.checked)">
                                                        </div>
                                                    </td>
                                                </tr>
                                                <?php $i++; ?>
...
<script>
    function updateActiveStatus(homeSecId, isChecked) {
        const status = isChecked ? 1 : 0;

        // Show loader overlay
        document.getElementById('loader-overlay').style.display = 'flex';

        fetch('/home-category/update-active-status', {
                method: 'POST',
                headers: {
